    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AdapterView.OnItemClickListener itemClickListener = new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                {
                    if(db.checkExercise("Przysiad",dbName))
                    {
                        db.insertExercise("Przysiad", "PRZYSIAD", dbName);
                ...
                {
                    if(db.checkExercise("Wykroki",dbName))
                    {
                        db.insertExercise("Wykroki", "WYKROKI", dbName);
                        db.organizeList(dbName);
                        addToast();
                    }
                    else sameToast();
                ...
                {
                    if(db.checkExercise("Zakroki",dbName))
                    {
                        db.insertExercise("Zakroki", "ZAKROKI", dbName);
                        db.organizeList(dbName);
                        addToast();
                    }
                    else sameToast();
                ...
                {
                    if(db.checkExercise("Suwnica",dbName))
                    {
                        db.insertExercise("Suwnica", "SUWNICA", dbName);
                        db.organizeList(dbName);
                        addToast();
                    }
                    else sameToast();
        ...
        ListView listView = (ListView) findViewById(R.id.legsList);
